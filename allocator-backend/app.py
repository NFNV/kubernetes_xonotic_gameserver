...
def extract_allocation_response(allocation: dict) -> dict:
    ...
    if not metadata.get("name"):
        raise ValueError("allocation resource name is missing")
...
def allocate():
    try:
        allocation = custom_objects_api.create_namespaced_custom_object(
            ...
            namespace=AGONES_NAMESPACE,
            plural=ALLOCATION_PLURAL,
            body=build_allocation_manifest(),
        )
    ...
    if not allocation_name:
        return jsonify({"error": "allocation_create_failed", "message": "allocation name missing"}), 500
