...
- The frontend now has a Tournament Management section for persisted tournaments, teams, rounds, tournament match records, backend-owned selected-tournament outcome summaries, explicit tournament finalization, first-phase single-elimination bracket columns, server assignment controls, manual result recording, and active-assignment admin controls
...
- Requested map/mode selection is now backed by a conservative compatibility matrix exposed through `GET /game-config/options`; normal allocation only allows verified combinations (`dm/xoylent`, `dm/stormkeep`, `dm/solarium`, `tdm/stormkeep`, `ctf/runningmanctf`, `duel/xoylent`, `ca/stormkeep`, `ca/xoylent`) and keeps all other `ctf`/`duel`/`ca` maps plus `dom`/`kh` deferred as experimental candidates until a controlled tournament allocation probe verifies each map/mode pair with `getstatus`
- Tournament map/mode verification probes now surface requested, expected, and parsed live map/mode values directly in the allocation response, keep the nested verification payload for detail, and include a clear failure reason when `getstatus` does not verify the requested pair; the probe script reads both old nested and newer flat response shapes
- Tournament Match result recording is manual and PostgreSQL-backed: operators record scores, winner, and optional notes, which marks the match `finished`; generated single-elimination matches advance winners into the configured next-match slot, while server release remains separate
- Tournament finalization is explicit and PostgreSQL-backed: `GET /tournaments/<id>/summary` computes progress, final score, champion candidate, runner-up, blockers, and active server assignment count; `POST /tournaments/<id>/finalize` marks the tournament `completed` and stores `winner_team_id` plus `completed_at` after the final match has a recorded winner
